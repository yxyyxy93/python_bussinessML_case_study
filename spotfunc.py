...
def age_percentages(df):
    ...
    return {y:x/categories.value_counts().sum() for x,y in zip(categories.value_counts(),categories.value_counts().keys())}
...
def get_years_on_key_playlists_dates(df,key_playlists,playlist_mapper):
        ...
        df_filter = df[df.playlist_id.isin(key_playlists.id.tolist())]
